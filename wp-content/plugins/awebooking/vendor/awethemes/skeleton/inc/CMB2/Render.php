...
class Render {
	...
	public function render_field( array $field_args, CMB2_Field $field ) {
		...
		// Append & prepend text.
		$append = trim( $field->get_param_callback_result( 'append' ) );
		$prepend = trim( $field->get_param_callback_result( 'prepend' ) );

		if ( $append || $prepend ) {
			echo '<div class="skeleton-input-group">';
			echo $prepend ? '<span class="skeleton-input-group__addon">' . $prepend . '</span>' : ''; // WPCS: XSS OK.
		}
		...
		if ( $append || $prepend ) {
			echo $append ? '<span class="skeleton-input-group__addon">' . $append . '</span>' : ''; // WPCS: XSS OK.
			echo '</div>';
		}
