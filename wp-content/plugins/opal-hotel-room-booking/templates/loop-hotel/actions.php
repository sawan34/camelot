...
<ul class="opalhotel-hotel-actions">
	...
	<?php if ( $galleries ) : ?>
	<li>
		<a href="<?php echo esc_url( get_the_post_thumbnail_url( $post->ID, 'full' ) ) ?>" class="gallery view-gallery" data-id="<?php echo esc_attr( get_the_ID() ) ?>"></a>
		<map name="<?php echo esc_attr( $uniqid ) ?>">
			<?php foreach ( $galleries as $id ) : $attach = wp_get_attachment_image_src( $id, 'full' ); ?>
				<area shape="rect" coords="6,11,72,73" href="<?php echo esc_url( isset( $attach[0] ) ? $attach[0] : ''  ) ?>" alt="<?php echo esc_attr( get_the_title( $id ) ) ?>">
			<?php endforeach; ?>
		</map>
	</li>
	<?php endif; ?>
	<?php if ( $video ) : ?>
		<li>
			<a href="<?php echo esc_url( $video ) ?>" class="video view-video" data-rel="prettyPhoto" data-id="<?php echo esc_attr( get_the_ID() ) ?>"></a>
		</li>
	<?php endif; ?>
</ul>