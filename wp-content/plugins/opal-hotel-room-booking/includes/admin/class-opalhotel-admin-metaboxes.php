...
class OpalHotel_Admin_MetaBoxes {
	...
	public function __construct() {
		...
		add_action( 'cmb2_admin_init', array( $this, 'hotels_offers_config' ), 12 );
	...
	public function hotels_offers_config() {
		$prefix = 'offer_';
        $cmb = new_cmb2_box( array(
            'id' 			=> 'opalhotel_hotel_offer',
            'title'		 	=> __( 'Offers Setting', 'opal-hotel-room-booking' ),
            'object_types' 	=> array( OPALHOTEL_CPT_OFF ), // post type
            'context' 		=> 'normal', //  'normal', 'advanced', or 'side'
            'priority' 		=> 'high', //  'high', 'core', 'default' or 'low'
            'show_names' 	=> true, // Show field names on the left
                ) );
        ...
        $cmb->add_field( array(
            'name'       	=> __( 'Sub title', 'opal-hotel-room-booking' ),
            'desc'       	=> __( 'Add subtitle for offer', 'opal-hotel-room-booking' ),
            'id'         	=> $prefix . 'subtitle',
			'type' 			=> 'text'
        ) );
        ...
        $group_id = $cmb->add_field( array(
            'id'                => $prefix . 'includes',
            'type'              => 'group',
            'description'       => __( 'Offer Include', 'opal-hotel-room-booking' ),
            'options'           => array(
                'group_title'   => 'Offer include {#}',
                'add_button'    => 'Add offer include',
                'remove_button' => 'Remove offer include',
                'sortable'      => true
            )
        ) );
        ...
        $cmb->add_group_field( $group_id, array(
            'id'            => $prefix . 'include',
            'name'          => __( 'Offer Include', 'opal-hotel-room-booking' ),
            'type'          => 'textarea_small'
        ) );
    }
