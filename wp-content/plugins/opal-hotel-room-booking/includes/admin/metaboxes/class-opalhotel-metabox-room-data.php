...
class OpalHotel_MetaBox_Room_Data {
			...
			<?php foreach ( $tab_panels as $key => $tab ) : ?>
				...
				<?php if ( $key === 'general' ) : ?>
					...
					<div id="general_room_data" class="opalhotel_room_data_panel active">
						...
						<!-- quantity -->
						<div class="opalhotel_field_group">
							<label for="featured"><?php esc_html_e( 'Featured', 'opal-hotel-room-booking' ); ?></label>
							<input type="hidden" name="_featured" value="0" />
							<input name="_featured" id="featured" class="opalhotel_input_field" type="checkbox" value="1" <?php checked( $opalhotel_room->featured, 1 ) ?>/>
							<span class="opalhotel_tiptip" data-tip="<?php esc_attr_e( 'Is featured?', 'opal-hotel-room-booking' ); ?>"><i class="fa fa-question-circle-o"></i></span>
						</div>
	...
	public static function save( $post_id, $post ) {
		...
		/* set featured */
		update_post_meta( $post_id, '_featured', absint( $_POST['_featured'] ) );
		/* set quantity */
